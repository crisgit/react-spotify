      ...
      {viewType === 'playlist' && (
        <div className='playlist-title-container'>
          ...
          <div className='playlist-info-container'>
            ...
            <p className='created-by'>Criado por: <span className='lighter-text'>{currentPlaylist.owner.display_name}</span> - {currentPlaylist.tracks.total} songs</p>
            <button
      ...
      {(headerTitle === 'Home') && (
        <div>
          ...
          <div className='browse-headers'>
            <p className={viewType === 'Genres' ? 'active' : ''} onClick={() => { fetchCategories(token); updateViewType('Genres'); updateHeaderTitle('Home'); }}>Gênero</p>
            <p className={viewType === 'New Releases' ? 'active' : ''} onClick={() => { fetchNewReleases(token); updateViewType('New Releases'); updateHeaderTitle('Home'); }}>Lançamentos</p>
            <p className={viewType === 'Featured' ? 'active' : ''} onClick={() => { fetchFeatured(token); updateViewType('Featured'); updateHeaderTitle('Home'); }}>Músicas</p>
          </div>
        </div>
      )}
    </div>
